...
def get_left_team_label(players_boxes, kits_colors, kits_clf):
  ...
  for i in range(len(players_boxes)):
    ...
    x1, y1, x2, y2 = map(int, players_boxes[i].xyxy.cpu().numpy()[0])
...
def annotate_video(frames_or_path, model):
    ...
    if not frames:
        # If frames are not provided, read the first frame from the video to get the size
        ret, frame = cap.read()
        if not ret:
        ...
        # Reset the video capture to read from the beginning
        cap.release()
        cap = cv2.VideoCapture(frames_or_path)
    ...
    while cap.isOpened():
        if frames is None:
            ...
            # Get frame directly from frames
            frame = frames[int(cap.get(cv2.CAP_PROP_POS_FRAMES))]
            success = True
...
if __name__ == "__main__":
    ...
    # Pass the video path to annotate_video function
